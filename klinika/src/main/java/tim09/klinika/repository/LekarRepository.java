...
public interface LekarRepository extends JpaRepository<Lekar, Long> {
	...
	@Modifying
	@Transactional
	@Query(value="delete from Korisnici l where l.korisnik_id=?1 and l.tip='LE' and"
			+ "(select count(pr) from pregled pr where pr.lekar_id=l.korisnik_id and pr.vreme > ?2)=0 and "
			+ "(select count(ope) from operacija ope where (select op.lekar_id from operisali op where op.lekar_id=l.korisnik_id and op.operacija_id=ope.operacija_id)=l.korisnik_id and ope.vreme > ?2)=0",nativeQuery = true)
	Integer deleteById(long id,long vreme);
}
