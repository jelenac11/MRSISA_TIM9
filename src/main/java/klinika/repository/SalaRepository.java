...
public interface SalaRepository extends JpaRepository<Sala, Long> {
	...
	@Query(value = "select * from sala s where s.sala_id=?1 and s.aktivan=true and"
			+ "(select count(pr) from pregled pr where pr.sala_id=s.sala_id and pr.otkazan=false and (pr.vreme >= ?2 or pr.vreme+pr.trajanje >= ?2) or (pr.pacijent_id is not null and pr.otkazan = false))=0 and "
			+ "(select count(op) from operacija op where op.otkazana=false and op.sala_id=s.sala_id and (op.vreme >= ?2 or op.vreme >= ?2))=0;", nativeQuery = true)
