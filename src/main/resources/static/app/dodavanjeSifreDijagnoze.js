Vue.component("dodavanje-sifre-dijagnoze", {
	data : function() {
		return {
			novaStavka : {
				sifra : "",
	...
	<div class="container d-flex justify-content-center">
		<div class="card mt-5" style="width: 47rem;">
			<h4 class="card-header">Nova šifra dijagnoze</h4>
			<div class="card-body">
				<form class="needs-validation mb-4" v-bind:class="{ 'was-validated': submitovano }" novalidate @submit.prevent="dodajStavku" id="forma-dodaj-stavku">
				  	...
				  	<div class="form-row">
				    	<div class="col">
							...
							<div class="invalid-feedback" id="dodavanjeInvalid">Niste uneli naziv dijagnoze.</div>
						</div>
				  	</div>
				...
				<router-link :to="{ name: 'sifrarnikDijagnoza' }" class="btn btn-secondary">Nazad</router-link>
			</div>
		</div>
	</div>
	`
				...
				.then(response => {
					...
					.then(response => {
						...
						if (this.uspesnoDodavanje) {
							this.$router.replace({ name: 'sifrarnikDijagnoza' });
						}
					})
					.catch(error => {
						console.log(error);
